...
class ShellExecutionHandler(IPythonHandler):
    async def run_command(self, command=None, stdin=None):
        ...
        try:
            ...
            _log_function(
                self, "Process failed to execute.", stderr, stdout, process.returncode
            )
            raise e
        else:
            if process.returncode != 0:
                _log_function(
                    self,
                    "Process exited with return code that was non-zero.",
                    stderr,
                    stdout,
                    process.returncode,
                )
    ...
    def request_log_function(self):
        self.log.debug(
            "self.request.body_arguments: " + str(self.request.body_arguments)
        )
        self.log.debug("self.request.body: " + str(self.request.body))
        self.log.debug(
            "self.request.query_arguments: " + str(self.request.query_arguments)
        )
    ...
    async def patch(self, command):
        ...
        if command == "hold" or command == "release":
            stdout, stderr = await self.run_command(
                " ".join(["scontrol", command, job_list])
            )
            self.finish()
        else:
    ...
    async def post(self):
        ...
        if scriptIs:
            if scriptIs == "path":
                ...
                raise Exception(
                    "The query argument scriptIs needs to be either 'path' or 'contents'."
                )
    ...
    async def get(self):
        ...
        for line in lines:
            ...
            if len(line.split(maxsplit=7)) == 8:
                ...
                data_list += [
                    [(html.escape(entry)).strip() for entry in line.split(maxsplit=7)]
                ]
            else:
                self.log.debug(
                    "The following line from squeue appears to be invalid, and will not be printed in the table:\n"
                    + line
                )
                continue
