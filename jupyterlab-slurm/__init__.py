...
def _jupyter_server_extension_paths():
    return [{"module": "jupyterlab-slurm"}]
...
def load_jupyter_server_extension(nb_server_app):
    ...
    scontrol_hub_route_pattern1 = create_full_route_pattern_from(
        "/hub/scontrol/(?P<command>.*)"
    )
    scontrol_hub_route_pattern2 = create_full_route_pattern_from(
        "/lab/user/scontrol/(?P<command>.*)"
    )
    scontrol_hub_route_pattern3 = create_full_route_pattern_from(
        "/lab/scontrol/(?P<command>.*)"
    )
